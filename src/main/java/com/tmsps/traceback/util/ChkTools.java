...
public class ChkTools {
	...
	public static Object getNumber(String reqParamValue, Class<?> paramType) {
		...
		if (paramType == char.class || paramType == Character.class) {
			...
			return ChkTools.isIngteger(reqParamValue) ? new Byte(reqParamValue) : (byte) 0;
		} else if (paramType == short.class || paramType == Short.class) {
			return ChkTools.isIngteger(reqParamValue) ? new Short(reqParamValue) : (short) 0;
		} else if (paramType == int.class || paramType == Integer.class) {
			return ChkTools.getInteger(reqParamValue);
		} else if (paramType == long.class || paramType == Long.class) {
			return ChkTools.isIngteger(reqParamValue) ? new Long(reqParamValue) : (long) 0;
		} else if (paramType == float.class || paramType == Float.class) {
			return ChkTools.isIngteger(reqParamValue) ? new Float(reqParamValue) : (float) 0;
		} else if (paramType == double.class || paramType == Double.class) {
			return ChkTools.isIngteger(reqParamValue) ? new Double(reqParamValue) : (double) 0;
	...
	public static boolean isBasicType(Class<?> paramType) {
		Class<?>[] basic = { Boolean.class, Character.class, Byte.class, Short.class, Integer.class, Long.class, Float.class, Double.class };
		for (Class<?> b : basic) {
