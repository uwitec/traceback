var edit_form_panel = Ext.create("Ext.form.Panel", {
	url : "/traceback/shop/ingoods/edit.htm",
	buttonAlign : "center",
...
function myEdit(kid) {
	Ext.Ajax.request({
		url : "/traceback/shop/ingoods/edit_form.htm?kid=" + kid,
		success : function(response) {
			var json = Ext.JSON.decode(response.responseText);
			edit_form_panel.getForm().reset();

			edit_form_panel.getForm().setValues(json);
			edit_form_panel.getForm().findField('dingoods_time').setValue(dateFormat_2(json.ingoods_time));
