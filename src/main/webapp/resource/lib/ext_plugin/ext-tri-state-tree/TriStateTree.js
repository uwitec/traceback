...
Ext.define('Ext.ux.tree.TriStateTreePanel', {
    ...
    setChildrenCheckedStatus: function (current) {
        ...
        if (current.parentNode) {
			...
			if(current.data.disabled === false)
				current.set('checked', parent.get('checked'));
			else{
				//if(parent.get('checked')!=current.get('checked'))
				//me.updateParentCheckedStatus(current);	
				return;
			}
        }
        ...
        var data = {
            ...
            children: [
                { text: "id2 homework ",  checked: false,    expanded: true,
                    id  : 2,
                    children: [
                    {       id  : 3,  checked: true, text: "id3 book report ",  leaf: true },
                    {       id  : 4,   checked: false, disabled:true, text: "id4 algebra ", leaf: true}
