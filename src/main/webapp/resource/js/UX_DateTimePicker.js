...
Ext.define('Ext.ux.DateTimePicker', {
	  ...
	  handleDateClick: function(e, t) {
		  ...
		  if(!me.disabled && t.dateValue && !Ext.fly(t.parentNode).hasCls(me.disabledCellCls)) {
			...
			Ext.each(Ext.query('.x-datepicker-today'),function(item){
				 var selClass = item.className;
			selClass = selClass.replace('x-datepicker-today',''); 
				item.className = selClass;});
			
			Ext.each(Ext.query('.x-datepicker-sel'),function(item){
				 var selClass = item.className;
			selClass = selClass.replace('x-datepicker-sel',''); 
				item.className = selClass;});
			// Ext.query('.x-datepicker-today')[0].className = selClass;
			 Ext.fly(Ext.fly(t).dom).addCls('x-datepicker-sel')
...
//			  delete me.doCancelFocus;
	  ...
	  selectToday: function() {
		  ...
		  if(btn && !btn.disabled) {
			  ...
			  me.setValue(new Date(new Date(me.activeDate)));// overwrite: fill time before setValue
			  me.fireEvent('select', me, me.value);
			  if(handler) {
				  handler.call(me.scope || me, me, me.value);
			  }
