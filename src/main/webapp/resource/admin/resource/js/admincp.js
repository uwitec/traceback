...
//布局换色设置
        ...
        $("div.bgSelector").toggle(300, function() {
            if ($(this).html() == '') {
                $(this).sColor({
                    ...
                    defaultItem: ($.cookie('bgColorSelectorPosition') != null) ? $.cookie('bgColorSelectorPosition') : 37  // 可选，第几个颜色的索引作为初始颜色，默认第1个颜色
                });
            }
        });//切换显示
    });    
    if ($.cookie('bgColorSelectorPosition') != null) {
        ...
        $('.sub_menu_li').css('background-color', bgColorSelectorColors[$.cookie('bgColorSelectorPosition')].c);
    } else {
        $('body').css('background-color', bgColorSelectorColors[37].c);
        $('.sub_menu_li').css('background-color', bgColorSelectorColors[37].c);
    } 
...
function openItem(param) {
	try{
	    ...
	    $('li[data-param="' + $this.parents('div[id^="admincpNavTabs_"]').attr('id').split('_')[1] + '"]').addClass('active');
	    $this.parent().addClass('active').parents('dl:first').addClass('active').parents('div:first').show();
	    if(param!=null && param.length>2){
	    	$('#workspace').attr('src', PATH + param+'?'+new Date());
	    }
